...
for a in pdtdir:
    if "_0621_" in a:
        c.execute("SELECT entity_id FROM entities WHERE numero_documento='"+a[:11]+"'")
        entityId=(c.fetchone())[0]
        ...
        pdtcsv=pdtcsv[1:].set_axis(strcolumns,axis="columns").rename(columns={"7":"periodo_tributario","13":"fecha_presentacion","58":"hora"}).drop(["2","5"],axis=1)
        pdtcsv["nro_orden"]=a[a.index("_",12)+1:a.index("_",17)]
        pdtcsv["entity_id"]=entityId
        pdtImport=pd.concat([pdtImport,pdtcsv],join='inner',ignore_index=True)      
pdtImport.to_excel("/home/raknaros/code/tabla09.xlsx")